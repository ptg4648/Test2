...
class ExerciseAdapter(val exerciseData: ArrayList<ExerciseData>) : RecyclerView.Adapter<ExerciseAdapter.ViewHolder>() {
    ...
    class ViewHolder(private val binding: ItmeExcerciseBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(exerciseData: ExerciseData) {
            binding.tvName.text = exerciseData.exerciseName
            binding.tvTime.text = exerciseData.exerciseTime
        }
    }
}